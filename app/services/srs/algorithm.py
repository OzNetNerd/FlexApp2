...
class SRSAlgorithmService:
    ...
    def calculate_next_interval(self, item: SRS, ui_rating: int) -> float:
        ...
        """
        self.logger.info(
            f"SRSAlgorithmService: Calculating next interval for item {item.id} with UI rating {ui_rating}")
        ...
        if item.review_count == 0 or item.interval <= 0:
            self.logger.info(
                f"SRSAlgorithmService: Item {item.id} is new or has invalid interval, applying initial learning steps")
            # Use graduated learning steps (10min → 1h → 6h → 1d)
            if fsrs_rating == 1:
                interval = MIN_INTERVAL  # 10 minutes
                self.logger.info(
                    f"SRSAlgorithmService: Rating 1 (Again) - setting interval to {interval:.4f} days (10 min)")
                return interval
            elif fsrs_rating == 2:
                interval = SHORT_INTERVAL  # 1 hour
                self.logger.info(
                    f"SRSAlgorithmService: Rating 2 (Hard) - setting interval to {interval:.4f} days (1 hour)")
                return interval
            elif fsrs_rating == 3:
                interval = MEDIUM_INTERVAL  # 6 hours
                self.logger.info(
                    f"SRSAlgorithmService: Rating 3 (Good) - setting interval to {interval:.4f} days (6 hours)")
                return interval
            else:
            ...
            if fsrs_rating == 1:
                ...
                self.logger.info(
                    f"SRSAlgorithmService: Rating 1 (Again) - resetting interval to {interval:.4f} days (1 hour)")
                return interval
            elif fsrs_rating == 2:
                interval = min(item.interval * HARD_MULTIPLIER, MAX_INTERVAL)
                self.logger.info(
                    f"SRSAlgorithmService: Rating 2 (Hard) - setting interval to {interval:.2f} days (x{HARD_MULTIPLIER})")
                return interval
            elif fsrs_rating == 3:
                interval = min(item.interval * GOOD_MULTIPLIER, MAX_INTERVAL)
                self.logger.info(
                    f"SRSAlgorithmService: Rating 3 (Good) - setting interval to {interval:.2f} days (x{GOOD_MULTIPLIER})")
                return interval
            else:
                interval = min(item.interval * EASY_MULTIPLIER, MAX_INTERVAL)
                self.logger.info(
                    f"SRSAlgorithmService: Rating 4 (Easy) - setting interval to {interval:.2f} days (x{EASY_MULTIPLIER})")
                return interval
    ...
    def calculate_new_ease_factor(self, item: SRS, ui_rating: int) -> float:
        ...
        """
        self.logger.info(
            f"SRSAlgorithmService: Calculating new ease factor for item {item.id} with UI rating {ui_rating}")
        fsrs_rating = UI_TO_FSRS_RATING.get(ui_rating, 1)  # Default to 1 if invalid
        ...
        if fsrs_rating == 1:
            ...
            self.logger.info(
                f"SRSAlgorithmService: Rating 1 (Again) - reducing ease by {FAIL_EASE_PENALTY} to {new_ease:.2f}")
            return new_ease
        elif fsrs_rating == 2:
            new_ease = max(MIN_EASE_FACTOR, current_ease - HARD_EASE_PENALTY)
            self.logger.info(
                f"SRSAlgorithmService: Rating 2 (Hard) - reducing ease by {HARD_EASE_PENALTY} to {new_ease:.2f}")
            return new_ease
        elif fsrs_rating == 3:
            ...
            self.logger.info(
                f"SRSAlgorithmService: Rating 4 (Easy) - increasing ease by {EASY_EASE_BONUS} to {new_ease:.2f}")
            return new_ease
    ...
    def schedule_review(self, item: SRS, rating: int) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"SRSAlgorithmService: Calculated next interval: {next_interval:.2f} days, new ease factor: {new_ease:.2f}")
        ...
        self.logger.info(
            f"SRSAlgorithmService: Prepared update data for item {item.id} → next in {next_interval:.2f}d, ef={new_ease:.2f}"
        )
        return update_data