...
def companies_dashboard():
    ...
    top_companies = db.session.query(
        Company, db.func.count(Company.opportunities).label('opportunity_count')
    ).outerjoin(Company.opportunities).group_by(Company.id).order_by(
    ...
    segments = [
        {
            ...
            "count": db.session.query(Company).join(Company.opportunities).group_by(Company.id).having(
                db.func.count(Company.opportunities) > 2).count(),
            "percentage": calculate_percentage(
                db.session.query(Company).join(Company.opportunities).group_by(Company.id).having(
                    db.func.count(Company.opportunities) > 2).count(),
                total_companies
            )
        },
        {
            ...
            "count": db.session.query(Company).join(Company.opportunities).group_by(Company.id).having(
                db.func.count(Company.opportunities).between(1, 2)).count(),
            "percentage": calculate_percentage(
                db.session.query(Company).join(Company.opportunities).group_by(Company.id).having(
                    db.func.count(Company.opportunities).between(1, 2)).count(),
                total_companies
        ...
        },
        {
            ...
            "count": db.session.query(Company).outerjoin(Company.opportunities).group_by(Company.id).having(
                db.func.count(Company.opportunities) == 0).count(),
            "percentage": calculate_percentage(
                db.session.query(Company).outerjoin(Company.opportunities).group_by(Company.id).having(
                    db.func.count(Company.opportunities) == 0).count(),
                total_companies
            )
        }
...
def statistics():
    ...
    no_engagement = db.session.query(Company).outerjoin(Company.opportunities).outerjoin(
        Company.contacts).group_by(Company.id).having(
        db.func.count(Company.opportunities) == 0,
        db.func.count(Company.contacts) == 0
    ).count()
