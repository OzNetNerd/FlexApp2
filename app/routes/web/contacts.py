...
def statistics():
    ...
    skill_distribution = db.session.query(
        Contact.skill_level, db.func.count(Contact.id).label('count')
    ).group_by(Contact.skill_level).all()

    # Get primary skill area distribution
    skill_area_distribution = db.session.query(
        Contact.primary_skill_area, db.func.count(Contact.id).label('count')
    ).filter(Contact.primary_skill_area.isnot(None)).group_by(Contact.primary_skill_area).all()
