...
class GenericWebRoutes(CRUDRoutesBase):
    ...
    def _validate_create(self, form_data):
        return [f"{field} is required" for field in self.required_fields if not form_data.get(field)]
    ...
    def add_view_context(self, item, context):
        ...
        relationship = next((rel for rel in getattr(item, "relationships", []) if rel.user_id == current_user.id), None)
        context['relationship'] = relationship
        context['crisp_scores'] = relationship.crisp_scores.order_by(CRISPScore.created_at.desc()).all() if relationship else []
