...
def filtered_tasks():
    ...
    filters = {"status": request.args.get("status"), "priority": request.args.get("priority"), "due_date": request.args.get("due_date")}

    # Get filtered tasks from service
    tasks = task_service.get_filtered_tasks(filters)

    # Create context for the filtered view
    context = WebContext(title="Filtered Tasks", read_only=True, tasks=tasks, filters=filters)
