...
from app.routes.api.clients.pages.companies import CompanyApiClient
...
company_api_client = CompanyApiClient()

@companies_bp.route("/", methods=["GET"], endpoint="index")
@login_required
def companies_dashboard():
    # Get statistics and data from API client instead of service
    stats = company_api_client.get_dashboard_stats()
    top_companies = company_api_client.get_top_companies()
    segments = company_api_client.get_engagement_segments()
    growth_data = company_api_client.get_growth_data()
