...
@login_required
def filtered_users():
    filters = {"is_admin": request.args.get("is_admin"), "period": request.args.get("period"), "activity": request.args.get("activity")}

    users = user_service.get_filtered_users(filters)

    context = WebContext(title="Filtered Users", read_only=True, users=users, filters=filters)
