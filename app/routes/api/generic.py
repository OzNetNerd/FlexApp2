...
class GenericAPIRoutes(CRUDRoutesBase):
    ...
    def _register_routes(self):
        ...
        self.blueprint.add_url_rule(
            "/<int:item_id>", "get", self._get_route, methods=["GET"]
        )
        ...
        # Update endpoint
        self.blueprint.add_url_rule(
            "/<int:item_id>", "update", self._update_route, methods=["PUT", "PATCH"]
        )

        # Delete endpoint
        self.blueprint.add_url_rule(
            "/<int:item_id>", "delete", self._delete_route, methods=["DELETE"]
        )
    ...
    def _list_route(self):
        ...
        logger.debug(
            f"Request: {request.path} | Args: {dict(request.args)} | Headers: {dict(request.headers)}"
        )
        ...
        try:
            ...
            for key, value in request.args.items():
                if key.startswith("filter."):
                    field = key.split(".")[1]
                    filter_model[field] = {"type": "contains", "filter": value}
    ...
    def _get_route(self, item_id):
        ...
        """
        logger.debug(
            f"Handling API get route for {self.model.__name__} with id {item_id}."
        )
        logger.debug(
            f"Request: {request.path} | Args: {dict(request.args)} | Headers: {dict(request.headers)}"
        )
        ...
        try:
            ...
            return jsonify(
                {
                    "error": (
                        "Item Not Found" if "404" in str(e) else "Internal Server Error"
                    ),
                    "message": str(e),
                }
    ...
    def _create_route(self):
        ...
        logger.debug(
            f"Request: {request.path} | Method: {request.method} | Headers: {dict(request.headers)}"
        )
        ...
        try:
            ...
            if validation_errors:
                ...
                return (
                    jsonify(
                        {"error": "Validation Error", "messages": validation_errors}
                    ),
                    400,
                )
    ...
    def _update_route(self, item_id):
        ...
        """
        logger.debug(
            f"Handling API update route for {self.model.__name__} with id {item_id}."
        )
        logger.debug(
            f"Request: {request.path} | Method: {request.method} | Headers: {dict(request.headers)}"
        )
    ...
    def _delete_route(self, item_id):
        ...
        """
        logger.debug(
            f"Handling API delete route for {self.model.__name__} with id {item_id}."
        )
        logger.debug(
            f"Request: {request.path} | Method: {request.method} | Headers: {dict(request.headers)}"
        )
        ...
        try:
            ...
            result = {"message": f"{self.model.__name__} deleted successfully"}
