...
class ItemManager:
    ...
    def get_item_by_id(self, item_id) -> Tuple[Optional[Any], Optional[str]]:
        ...
        try:
            ...
            error_msg = (
                f"Error accessing {self.model.__name__} with id {item_id}: {str(e)}"
            )
    ...
    def create_item(self, form_data):
        ...
        try:
            logger.debug(
                f"Creating new {self.model.__name__} with data: {list(form_data.keys())}"
            )
    ...
    def update_item(self, item, form_data):
        ...
        try:
            logger.debug(
                f"Updating {self.model.__name__} with id {item.id}, fields: {list(form_data.keys())}"
            )
            ...
            return (
                redirect(url_for(f"{self.blueprint_name}.view", item_id=item.id)),
                None,
            )
        except Exception as e:
            logger.error(
                f"Error updating {self.model.__name__} with id {item.id}: {str(e)}"
            )
            logger.error(f"Traceback: {traceback.format_exc()}")
    ...
    def delete_item(self, item):
        ...
        try:
            ...
            logger.error(
                f"Error deleting {self.model.__name__} with id {item.id}: {str(e)}"
            )
            logger.error(f"Traceback: {traceback.format_exc()}")
