...
// Update the Next button's href with the appropriate URL
function updateNextButtonHref() {
  const nextItemId = APP_CONFIG.nextItemId;
  if (nextItemId) {
    const nextBtn = document.querySelector('#next-btn-wrapper a');
    if (nextBtn) {
      nextBtn.setAttribute('href', `/srs/review/${nextItemId}`);
    }
  }
}
...
document.addEventListener('keydown', function (event) {
  ...
  if (event.shiftKey && event.key === 'ArrowRight') {
    const nextBtn = document.querySelector('#next-btn-wrapper a');
    if (nextBtn && !nextBtn.parentElement.classList.contains('d-none')) {
...
document.getElementById('review-form').addEventListener('submit', function (e) {
  ...
  // Update next button's href and show it
  updateNextButtonHref();
  document.getElementById('next-btn-wrapper').classList.remove('d-none');
});

// Function for submitting the review when Next button is clicked
...
async function submitReviewToServer(answerGiven, rating) {
