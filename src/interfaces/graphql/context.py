...
from src.infrastructure.auth.services import get_current_user
...
def get_graphql_context(request_context=None) -> GraphQLContext:
    ...
    return GraphQLContext(request=req, unit_of_work=unit_of_work, loaders=DataLoaderContext(**loaders), user=user)
