...
* **Mechanism (Web - `app/routes/web/route_registration.py`):**
    ...
    6.  For GET requests, it determines the appropriate context class (`TableContext` for 'index', `EntityContext` for 'view'/'edit'/'create') and template path.
...
* **Mechanism:**
    1.  You decorate your route handler (e.g., `def view(context, entity_id):`) with `@use_context(EntityContext, entity_table_name="Company", action="view")`.
    2.  When a request hits the route, Flask calls the decorator *before* your `view` function.
    3.  The decorator inspects its arguments (`EntityContext`, `entity_table_name`, `action`).
    4.  It identifies that it needs an entity (because it's using `EntityContext` and the action is 'view' or 'edit').
    ...
    8.  It instantiates `EntityContext`, passing the fetched entity, the action ('view'), and other necessary details.
...
* **Web Context (`TableContext`, `EntityContext`):**
    ...
    * `TableContext` is specialized for list views (index pages), setting up data needed by `_table_index.html` (like `api_url`, `table_id`).
    * `EntityContext` is for single-item views/forms, holding the specific `entity` object and action type.
...
│   │   ├── context.py  # Web context classes (TableContext, EntityContext)
...
* **Web CRUD Registration (`app/routes/web/route_registration.py`):**
    ...
    * It uses a generic `route_handler` which determines the correct context (`TableContext` or `EntityContext`) and template based on the endpoint (`index`, `create`, `view`, `edit`, `delete`).
...
* **Context Injection (`@use_context` in `app/routes/web/context_utils.py`):**
    * This decorator simplifies view functions by automatically creating and injecting the appropriate context object (`TableContext`, `EntityContext`) based on the provided class and arguments.
...
* **Context Injection:** (`@use_context`, `BaseContext`, `TableContext`, `EntityContext`) Standardizes data passing from routes to web templates.
